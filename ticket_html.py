...
def parse_html(html):
    ...
    items = soup.find_all("li", class_="offline-order-detail__product-item")
    ...
    for item in items:
        # Extract name
        name_tag = item.select_one(".offline-order-detail__product-item-label > p")
        name = name_tag.get_text(strip=True) if name_tag else ""

        # Extract quantity
        quantity_tag = item.select_one(".offline-order-detail__product-item-quantity")
        quantity_text = quantity_tag.get_text(strip=True) if quantity_tag else "x1"
        quantity = (
            int(re.search(r"x(\d+)", quantity_text).group(1))
            if re.search(r"x(\d+)", quantity_text)
            else 1
        )
    ...
    return list(zip(names, quantities, prices)), round(total_price, 2)
...
class ShoppingApp:
    ...
    def load_items(self):
        ...
        items, self.total_price = parse_html(raw_text)
